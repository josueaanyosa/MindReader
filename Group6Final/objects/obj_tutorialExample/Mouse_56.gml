/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 5814AE5D
/// @DnDArgument : "var" "tutorialStep"
/// @DnDArgument : "value" "9"
if(tutorialStep == 9)
{
	/// @DnDAction : YoYo Games.Rooms.Previous_Room
	/// @DnDVersion : 1
	/// @DnDHash : 67360475
	/// @DnDParent : 5814AE5D
	room_goto_previous();
}

/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 1CDCBA1C
/// @DnDArgument : "var" "tutorialStep"
/// @DnDArgument : "value" "8"
if(tutorialStep == 8)
{
	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 77D57454
	/// @DnDParent : 1CDCBA1C
	/// @DnDArgument : "expr" "9"
	/// @DnDArgument : "var" "tutorialStep"
	tutorialStep = 9;

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 530EFFF9
	/// @DnDParent : 1CDCBA1C
	/// @DnDArgument : "expr" ""This is the end of the tutorial.""
	/// @DnDArgument : "var" "tutorialInstructions1"
	tutorialInstructions1 = "This is the end of the tutorial.";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 0147A72E
	/// @DnDParent : 1CDCBA1C
	/// @DnDArgument : "expr" ""Now you're ready to be a mind reader!""
	/// @DnDArgument : "var" "tutorialInstructions2"
	tutorialInstructions2 = "Now you're ready to be a mind reader!";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 3EB2E686
	/// @DnDParent : 1CDCBA1C
	/// @DnDArgument : "expr" ""Use your left mouse button again to""
	/// @DnDArgument : "var" "tutorialInstructions3"
	tutorialInstructions3 = "Use your left mouse button again to";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 1D6E017A
	/// @DnDParent : 1CDCBA1C
	/// @DnDArgument : "expr" ""return back to the level screen.""
	/// @DnDArgument : "var" "tutorialInstructions4"
	tutorialInstructions4 = "return back to the level screen.";
}

/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 47BCC4EE
/// @DnDArgument : "var" "tutorialStep"
/// @DnDArgument : "value" "7"
if(tutorialStep == 7)
{
	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 563ED61C
	/// @DnDParent : 47BCC4EE
	/// @DnDArgument : "expr" "8"
	/// @DnDArgument : "var" "tutorialStep"
	tutorialStep = 8;

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 371F60CA
	/// @DnDParent : 47BCC4EE
	/// @DnDArgument : "expr" "5"
	/// @DnDArgument : "var" "tutorialScore"
	tutorialScore = 5;

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 241F6B80
	/// @DnDParent : 47BCC4EE
	/// @DnDArgument : "expr" ""The level ends when your score is 5,""
	/// @DnDArgument : "var" "tutorialInstructions1"
	tutorialInstructions1 = "The level ends when your score is 5,";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 54C39602
	/// @DnDParent : 47BCC4EE
	/// @DnDArgument : "expr" ""so you will see a Level Complete""
	/// @DnDArgument : "var" "tutorialInstructions2"
	tutorialInstructions2 = "so you will see a Level Complete";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 45FD1FA9
	/// @DnDParent : 47BCC4EE
	/// @DnDArgument : "expr" ""screen and can choose another level.""
	/// @DnDArgument : "var" "tutorialInstructions3"
	tutorialInstructions3 = "screen and can choose another level.";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 7CB5E5E5
	/// @DnDParent : 47BCC4EE
	/// @DnDArgument : "expr" "" ""
	/// @DnDArgument : "var" "tutorialInstructions4"
	tutorialInstructions4 = " ";
}

/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 494C733F
/// @DnDArgument : "var" "tutorialStep"
/// @DnDArgument : "value" "6"
if(tutorialStep == 6)
{
	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 409C6F12
	/// @DnDParent : 494C733F
	/// @DnDArgument : "expr" "7"
	/// @DnDArgument : "var" "tutorialStep"
	tutorialStep = 7;

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 16CECFC1
	/// @DnDParent : 494C733F
	/// @DnDArgument : "expr" ""You can use the back or home buttons to""
	/// @DnDArgument : "var" "tutorialInstructions1"
	tutorialInstructions1 = "You can use the back or home buttons to";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 786D0496
	/// @DnDParent : 494C733F
	/// @DnDArgument : "expr" ""leave your current level at any time,""
	/// @DnDArgument : "var" "tutorialInstructions2"
	tutorialInstructions2 = "leave your current level at any time,";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 742555C5
	/// @DnDParent : 494C733F
	/// @DnDArgument : "expr" ""but you will lose your level progress""
	/// @DnDArgument : "var" "tutorialInstructions3"
	tutorialInstructions3 = "but you will lose your level progress";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 083518A1
	/// @DnDParent : 494C733F
	/// @DnDArgument : "expr" ""if you do so!""
	/// @DnDArgument : "var" "tutorialInstructions4"
	tutorialInstructions4 = "if you do so!";
}

/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 3747DA2A
/// @DnDArgument : "var" "tutorialStep"
/// @DnDArgument : "value" "5"
if(tutorialStep == 5)
{
	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 65EC6CE1
	/// @DnDParent : 3747DA2A
	/// @DnDArgument : "expr" "6"
	/// @DnDArgument : "var" "tutorialStep"
	tutorialStep = 6;

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 2D998C24
	/// @DnDParent : 3747DA2A
	/// @DnDArgument : "expr" ""Once per level, you can use the hint""
	/// @DnDArgument : "var" "tutorialInstructions1"
	tutorialInstructions1 = "Once per level, you can use the hint";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 426BE841
	/// @DnDParent : 3747DA2A
	/// @DnDArgument : "expr" ""button to have an incorrect option""
	/// @DnDArgument : "var" "tutorialInstructions2"
	tutorialInstructions2 = "button to have an incorrect option";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 27A18818
	/// @DnDParent : 3747DA2A
	/// @DnDArgument : "expr" ""removed. When doing this, your score""
	/// @DnDArgument : "var" "tutorialInstructions3"
	tutorialInstructions3 = "removed. When doing this, your score";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 575C4DCB
	/// @DnDParent : 3747DA2A
	/// @DnDArgument : "expr" ""will decrease by 1.""
	/// @DnDArgument : "var" "tutorialInstructions4"
	tutorialInstructions4 = "will decrease by 1.";

	/// @DnDAction : YoYo Games.Instances.Destroy_Instance
	/// @DnDVersion : 1
	/// @DnDHash : 7B17CDC9
	/// @DnDApplyTo : {obj_hintForTutorial}
	/// @DnDParent : 3747DA2A
	with(obj_hintForTutorial) instance_destroy();

	/// @DnDAction : YoYo Games.Instances.Destroy_Instance
	/// @DnDVersion : 1
	/// @DnDHash : 0F3090EA
	/// @DnDApplyTo : {obj_buttonSadForTutorial}
	/// @DnDParent : 3747DA2A
	with(obj_buttonSadForTutorial) instance_destroy();
}

/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 2042BC38
/// @DnDArgument : "var" "tutorialStep"
/// @DnDArgument : "value" "4"
if(tutorialStep == 4)
{
	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 27B06605
	/// @DnDParent : 2042BC38
	/// @DnDArgument : "expr" "5"
	/// @DnDArgument : "var" "tutorialStep"
	tutorialStep = 5;

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 09E353F1
	/// @DnDParent : 2042BC38
	/// @DnDArgument : "var" "tutorialScore"
	tutorialScore = 0;

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 33F15C24
	/// @DnDParent : 2042BC38
	/// @DnDArgument : "expr" ""Your score will also go down by 1,""
	/// @DnDArgument : "var" "tutorialInstructions1"
	tutorialInstructions1 = "Your score will also go down by 1,";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 3C76B292
	/// @DnDParent : 2042BC38
	/// @DnDArgument : "expr" ""but you get more chances to guess the""
	/// @DnDArgument : "var" "tutorialInstructions2"
	tutorialInstructions2 = "but you get more chances to guess the";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 69343873
	/// @DnDParent : 2042BC38
	/// @DnDArgument : "expr" ""right answer and get your point back!""
	/// @DnDArgument : "var" "tutorialInstructions3"
	tutorialInstructions3 = "right answer and get your point back!";
}

/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 113A4895
/// @DnDArgument : "var" "tutorialStep"
/// @DnDArgument : "value" "3"
if(tutorialStep == 3)
{
	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 0AEA4D19
	/// @DnDParent : 113A4895
	/// @DnDArgument : "expr" "4"
	/// @DnDArgument : "var" "tutorialStep"
	tutorialStep = 4;

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 1A864E00
	/// @DnDParent : 113A4895
	/// @DnDArgument : "expr" ""If a wrong answer is chosen (sad),""
	/// @DnDArgument : "var" "tutorialInstructions1"
	tutorialInstructions1 = "If a wrong answer is chosen (sad),";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 2E3622AC
	/// @DnDParent : 113A4895
	/// @DnDArgument : "expr" ""an incorrect graphic will appear.""
	/// @DnDArgument : "var" "tutorialInstructions2"
	tutorialInstructions2 = "an incorrect graphic will appear.";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 5D9A1CC8
	/// @DnDParent : 113A4895
	/// @DnDArgument : "expr" "" ""
	/// @DnDArgument : "var" "tutorialInstructions3"
	tutorialInstructions3 = " ";

	/// @DnDAction : YoYo Games.Instances.Create_Instance
	/// @DnDVersion : 1
	/// @DnDHash : 57BF16C6
	/// @DnDParent : 113A4895
	/// @DnDArgument : "xpos" "675"
	/// @DnDArgument : "ypos" "70"
	/// @DnDArgument : "objectid" "obj_incorrect"
	/// @DnDArgument : "layer" ""Instances_1""
	/// @DnDSaveInfo : "objectid" "obj_incorrect"
	instance_create_layer(675, 70, "Instances_1", obj_incorrect);
}

/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 5B8AE1A6
/// @DnDArgument : "var" "tutorialStep"
/// @DnDArgument : "value" "2"
if(tutorialStep == 2)
{
	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 40AFC482
	/// @DnDParent : 5B8AE1A6
	/// @DnDArgument : "expr" "3"
	/// @DnDArgument : "var" "tutorialStep"
	tutorialStep = 3;

	/// @DnDAction : YoYo Games.Instances.Set_Sprite
	/// @DnDVersion : 1
	/// @DnDHash : 0CBCA973
	/// @DnDParent : 5B8AE1A6
	/// @DnDArgument : "spriteind" "spr_happy4"
	/// @DnDSaveInfo : "spriteind" "spr_happy4"
	sprite_index = spr_happy4;
	image_index = 0;

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 204E0885
	/// @DnDParent : 5B8AE1A6
	/// @DnDArgument : "expr" "1"
	/// @DnDArgument : "var" "tutorialScore"
	tutorialScore = 1;

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 75CF0D8E
	/// @DnDParent : 5B8AE1A6
	/// @DnDArgument : "expr" ""Your score in the top left corner""
	/// @DnDArgument : "var" "tutorialInstructions1"
	tutorialInstructions1 = "Your score in the top left corner";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 664E3455
	/// @DnDParent : 5B8AE1A6
	/// @DnDArgument : "expr" ""will also go up by 1 and""
	/// @DnDArgument : "var" "tutorialInstructions2"
	tutorialInstructions2 = "will also go up by 1 and";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 17970956
	/// @DnDParent : 5B8AE1A6
	/// @DnDArgument : "expr" ""a new face will appear.""
	/// @DnDArgument : "var" "tutorialInstructions3"
	tutorialInstructions3 = "a new face will appear.";
}

/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 248E9AA1
/// @DnDArgument : "var" "tutorialStep"
/// @DnDArgument : "value" "1"
if(tutorialStep == 1)
{
	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 10FB06B2
	/// @DnDParent : 248E9AA1
	/// @DnDArgument : "expr" "2"
	/// @DnDArgument : "var" "tutorialStep"
	tutorialStep = 2;

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 1A5B28D6
	/// @DnDParent : 248E9AA1
	/// @DnDArgument : "expr" ""If you guessed the correct answer ""
	/// @DnDArgument : "var" "tutorialInstructions1"
	tutorialInstructions1 = "If you guessed the correct answer ";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 0BDAC579
	/// @DnDParent : 248E9AA1
	/// @DnDArgument : "expr" ""(happy), a correct answer graphic""
	/// @DnDArgument : "var" "tutorialInstructions2"
	tutorialInstructions2 = "(happy), a correct answer graphic";

	/// @DnDAction : YoYo Games.Common.Variable
	/// @DnDVersion : 1
	/// @DnDHash : 0C1AD3D1
	/// @DnDParent : 248E9AA1
	/// @DnDArgument : "expr" ""will appear.""
	/// @DnDArgument : "var" "tutorialInstructions3"
	tutorialInstructions3 = "will appear.";

	/// @DnDAction : YoYo Games.Instances.Create_Instance
	/// @DnDVersion : 1
	/// @DnDHash : 3BF1F94C
	/// @DnDParent : 248E9AA1
	/// @DnDArgument : "xpos" "675"
	/// @DnDArgument : "ypos" "70"
	/// @DnDArgument : "objectid" "obj_correct"
	/// @DnDArgument : "layer" ""Instances_1""
	/// @DnDSaveInfo : "objectid" "obj_correct"
	instance_create_layer(675, 70, "Instances_1", obj_correct);
}